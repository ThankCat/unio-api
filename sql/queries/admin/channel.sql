...
WHERE (sqlc.narg('provider_id')::bigint IS NULL OR c.provider_id = sqlc.narg('provider_id')::bigint)
  ...
  AND (
    ...
    OR p.origin ILIKE '%' || sqlc.narg('q')::text || '%'
  )
...
WITH matching AS MATERIALIZED (
    ...
    WHERE c.id = sqlc.arg(channel_id)
      ...
      AND p.origin_revision = sqlc.arg(expected_origin_revision)
      AND p.status_revision = sqlc.arg(expected_status_revision)
    FOR UPDATE OF c
), applied AS (
    UPDATE channels AS c
    ...
    SELECT
        ...
        sqlc.arg(expected_origin_revision),
        sqlc.arg(expected_status_revision),
...
-- name: CommitChannelCapacityAtRevision :one
...
UPDATE channels
SET concurrency_limit = sqlc.narg(concurrency_limit),
    capacity_revision = sqlc.arg(next_revision),
    updated_at = now()
WHERE id = sqlc.arg(id)
...
WITH deleted_channel_price_service_tiers AS (
    ...
    WHERE channel_price_service_tiers.channel_price_id IN (
        SELECT channel_prices.id FROM channel_prices WHERE channel_prices.channel_id = sqlc.arg(id)
    )
),
...
WITH cache_usage AS (
    ...
    WHERE r.final_channel_id = sqlc.arg('channel_id')
