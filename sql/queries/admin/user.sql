...
ORDER BY
  ...
  CASE WHEN sqlc.narg('sort_field')::text = 'total_topup' AND COALESCE(sqlc.narg('sort_desc')::bool, false) THEN COALESCE((SELECT SUM(le.amount) FROM ledger_entries le WHERE le.user_id = u.id AND le.entry_type IN ('credit', 'cdkey_credit', 'adjustment_credit') AND le.currency = 'USD'), 0) END DESC NULLS LAST,
  CASE WHEN sqlc.narg('sort_field')::text = 'total_topup' AND NOT COALESCE(sqlc.narg('sort_desc')::bool, false) THEN COALESCE((SELECT SUM(le.amount) FROM ledger_entries le WHERE le.user_id = u.id AND le.entry_type IN ('credit', 'cdkey_credit', 'adjustment_credit') AND le.currency = 'USD'), 0) END ASC NULLS LAST,
...
GROUP BY k.id, k.name, k.key_prefix, k.user_id, k.disabled_at, k.revoked_at, k.expires_at, k.spend_limit, k.spent_total, k.last_used_at
ORDER BY
