...
const (
	providerSlug = "codex-e2e-fake"
	channelName  = "codex-pool-fake"
	modelID      = "gpt-5.5"
	userEmail    = "codex-e2e@unio.local" // 与真实种子同一个用户，复用 API Key
)
...
func main() {
	...
	err = pool.QueryRow(ctx, `
		...
		VALUES ($1, 'Codex 假上游 E2E', 'http://127.0.0.1:18998', 'enabled', 'USD')
	...
	err = pool.QueryRow(ctx, `
		...
		VALUES ($1, $2, ARRAY['openai'], 'codex', '', 'enabled', 0, 'pool', 3)
