-- 基准价与售价分开配置：允许只有基准价、没有售价的草稿行。
--
-- 可售不变量改由应用层与运行时守住，不再用 CHECK 拦在库外：
--   模型可售 = 有生效基准价 AND (倍率售价 OR 绝对售价)
-- 草稿行可以落库，但不可启用、不可出现在 /v1/models、请求打不出候选。
-- 毛利守卫在售价不可解析时跳过该行（不是判亏本）；配了售价后仍逐项拒绝售价 < 成本。
