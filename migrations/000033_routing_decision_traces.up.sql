...
CREATE TABLE public.routing_decision_traces (
    ...
    algorithm_version text DEFAULT 'objective_v1'::text NOT NULL,
    trace_status text DEFAULT 'partial'::text NOT NULL,
    ...
    CONSTRAINT routing_decision_traces_capacity_wait_result_check CHECK (((capacity_wait_result IS NULL) OR (capacity_wait_result = ANY (ARRAY['acquired'::text, 'capacity_exhausted'::text, 'rate_limited'::text, 'canceled'::text, 'not_waited'::text])))),
    ...
    CONSTRAINT routing_decision_traces_final_result_check CHECK (((final_result IS NULL) OR (final_result = ANY (ARRAY['success'::text, 'client_canceled'::text, 'capacity_exhausted'::text, 'rate_limited'::text, 'no_available_channel'::text, 'upstream_failed'::text, 'gateway_error'::text])))),
    CONSTRAINT routing_decision_traces_payload_object CHECK ((jsonb_typeof(trace_payload) = 'object'::text)),
    ...
    CONSTRAINT routing_decision_traces_sticky_action_check CHECK (((sticky_action IS NULL) OR (sticky_action = ANY (ARRAY['disabled'::text, 'miss'::text, 'hit'::text, 'bind_if_absent'::text, 'refresh_if_current'::text, 'clear_if_current'::text, 'preserve_on_temporary_bypass'::text, 'cas_conflict'::text, 'store_unavailable'::text]))))
);

ALTER SEQUENCE public.routing_decision_traces_id_seq OWNED BY public.routing_decision_traces.id;
ALTER TABLE ONLY public.routing_decision_traces ALTER COLUMN id SET DEFAULT nextval('public.routing_decision_traces_id_seq'::regclass);

ALTER TABLE ONLY public.routing_decision_traces
    ADD CONSTRAINT routing_decision_traces_pkey PRIMARY KEY (id);

ALTER TABLE ONLY public.routing_decision_traces
    ADD CONSTRAINT routing_decision_traces_request_key UNIQUE (request_record_id);
