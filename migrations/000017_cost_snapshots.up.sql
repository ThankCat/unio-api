...
CREATE TABLE public.cost_snapshots (
    ...
    tier_cost_source text,
    CONSTRAINT ck_cost_snapshots_service_tier CHECK (((service_tier IS NULL) OR (service_tier = ANY (ARRAY['standard'::text, 'fast'::text])))),
    CONSTRAINT ck_cost_snapshots_tier_cost_source CHECK (((tier_cost_source IS NULL) OR (tier_cost_source = ANY (ARRAY['derived'::text, 'absolute'::text])))),
