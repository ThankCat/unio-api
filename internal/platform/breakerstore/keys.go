...
// providerEvidence 保存 Origin HTTP 500/timeout 跨渠道、跨模型证据的短 TTL 集合（§5.1）。
// category 只能是 http_500 | first_token_timeout | body_read_timeout。
func (k keyBuilder) providerEvidenceChannels(id int64, category string) string {
	return k.prefix + "provider-evidence:" + strconv.FormatInt(id, 10) + ":" + category + ":channels"
}

func (k keyBuilder) providerEvidenceModels(id int64, category string) string {
	return k.prefix + "provider-evidence:" + strconv.FormatInt(id, 10) + ":" + category + ":models"
}
