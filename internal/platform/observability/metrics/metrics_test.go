...
func TestMetricsExposeBoundedRuntimeFacts(t *testing.T) {
	...
	m.IncBreakerSkip("provider", "open")
	...
	m.IncBreakerIgnoredResult("provider", "stale_revision")
	m.IncChannelConfigRevisionMismatch("finish")
	m.IncChannelCredentialRotationVerification("passed")
	m.SetOriginRevisionFence("23", "pending", 3*time.Second)
	m.SetProviderStatusRevisionFence("23", "active", 0)
	m.IncProviderStatusRevisionMismatch("acquire")
	...
	m.IncProviderFailure("23", "http_500")
	m.IncChannelFailure("17", "server")
	m.ObserveUpstreamTiming("23", "17", "openai", "responses", "stream", time.Second, &ttft)
	m.ObserveUpstreamTiming("23", "17", "openai", "responses", "non_stream", 2*time.Second, nil)
	...
	wants := []string{
		...
		`unio_gateway_breaker_skip_total{reason="open",scope="provider"} 1`,
		...
		`unio_gateway_breaker_ignored_result_total{reason="stale_revision",scope="provider"} 1`,
		...
		`unio_gateway_origin_revision_pending_seconds{provider_id="23"} 3`,
		`unio_gateway_provider_status_revision_fence{provider_id="23",state="active"} 1`,
		`unio_gateway_provider_status_revision_mismatch_total{operation="acquire"} 1`,
		...
		`unio_gateway_provider_failure_total{category="http_500",provider_id="23"} 1`,
