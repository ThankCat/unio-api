...
// v2：新增解析 cache_write_tokens（GPT-5.6+），从 uncached 拆出并计入 30m 缓存创建维度。
...
func responseFacts(responseID, upstreamModel, rawFinish string, serviceTier *string, u adapter.ChatUsage, meta adapter.UpstreamMetadata, source usage.Source) adapter.ResponseFacts {
	return adapter.ResponseFacts{
		...
		ServiceTier:         servicetier.ResolveOpenAIResponse(serviceTier),
		Metadata:            meta,
	}
}

// responseFactsNonStream 构造非流式响应的 ResponseFacts。
func responseFactsNonStream(responseID, upstreamModel, rawFinish string, serviceTier *string, u adapter.ChatUsage, meta adapter.UpstreamMetadata) adapter.ResponseFacts {
	return responseFacts(responseID, upstreamModel, rawFinish, serviceTier, u, meta, usage.SourceUpstreamResponse)
}

// responseFactsStream 构造流式响应的最终 ResponseFacts。
func responseFactsStream(responseID, upstreamModel, rawFinish string, serviceTier *string, u adapter.ChatUsage, meta adapter.UpstreamMetadata) adapter.ResponseFacts {
	return responseFacts(responseID, upstreamModel, rawFinish, serviceTier, u, meta, usage.SourceUpstreamStream)
}
...
func streamOutcome(responseID, upstreamModel, rawFinish string, serviceTier *string, finalUsage *adapter.ChatUsage, meta adapter.UpstreamMetadata) adapter.StreamOutcome {
	if finalUsage == nil {
		return adapter.StreamOutcome{}
	}

	facts := responseFactsStream(responseID, upstreamModel, rawFinish, serviceTier, *finalUsage, meta)
