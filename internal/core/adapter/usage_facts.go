...
//   - 输入拆成三段：命中 cache 读（CacheRead）、写入 cache（CacheCreation30m）、余下未缓存（Uncached）；
//   - OpenAI GPT-5.6+ 的 cache_write_tokens 是「本次处理并写入缓存」的 token，属 uncached 的子集，
//     按未缓存输入价 1.25x 计费，故从 uncached 中扣出、单列到 30m 缓存创建维度；旧模型/DeepSeek 不返回
...
func (u ChatUsage) ToUsageFacts() usage.Facts {
	...
	return usage.Facts{
		...
		CacheCreation30mInputTokens: usage.KnownTokens(int64(u.CacheWriteTokens)),
