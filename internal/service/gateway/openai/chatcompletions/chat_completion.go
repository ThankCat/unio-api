...
func (s *ChatCompletionService) CreateChatCompletion(ctx context.Context, req gatewayapi.ChatCompletionRequest) (*lifecycle.NonStreamResult[*gatewayapi.ChatCompletionResponse], error) {
	...
	requestParams, err := s.prepareRequestRecord(ctx, principal, req, false, tierRequest.Tier)
	if err != nil {
		return nil, err
	}
	...
	if stickyHint.Key == "" {
		// 内容派生兜底（第三节）：无显式会话信号的通用 chat 客户端按消息前缀哈希聚合会话。
		stickyHint = chatContentHint(req)
	}
	...
	candidatePlan, err := s.prepareChatCandidates(ctx, req, plan.Candidates, false, stickySession.BoundChannelID())
	if err != nil {
		requestRecord, createErr := s.lifecycle.CreatePreparedRequest(ctx, requestParams)
		if createErr != nil {
			return nil, createErr
	...
	runResult, err := s.attemptRunner.RunNonStream(ctx, lifecycle.RunNonStreamParams{
		...
		Invoke: func(ctx context.Context, candidate routing.ChatRouteCandidate) (lifecycle.AttemptSuccess, error) {
			...
			resp, err := chatAdapter.ChatCompletions(adapterCtx, candidate.Channel,
				mapGatewayRequestToAdapter(attemptReq, candidate.UpstreamModel))
