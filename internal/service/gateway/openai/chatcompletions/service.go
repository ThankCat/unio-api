...
// SetCapacityWaitTimeout 注入全池短等预算（gateway.capacity_wait_timeout_ms，§9.4）；<=0 关闭短等。
func (s *ChatCompletionService) SetCapacityWaitTimeout(d time.Duration) {
	s.attemptRunner.SetCapacityWaitTimeout(d)
}
...
func (s *ChatCompletionService) SetChannelSampleRecorder(recorder lifecycle.ChannelSampleRecorder) {
...
func (s *ChatCompletionService) SetAccountOutbound(resolver lifecycle.AccountOutboundResolver, sink lifecycle.AccountHealthSink) {
...
func (s *ChatCompletionService) SetTPMObserver(observer *tpmobserver.Observer) {
