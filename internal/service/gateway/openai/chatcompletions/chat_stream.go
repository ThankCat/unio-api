...
func (s *ChatCompletionService) StreamChatCompletion(ctx context.Context, req dto.ChatCompletionRequest, emit func(dto.ChatCompletionStreamResponse) error) error {
	...
	runResult, err := s.attemptRunner.RunStream(ctx, lifecycle.RunStreamParams{
		...
		EmitChunk: func(chunk chatcompletionsadapter.ChatStreamChunk, acks lifecycle.StreamWriteAcks) error {
		...
		Finish: func(streamID string, finalUsage *adapter.ChatUsage, _ string, acks lifecycle.StreamWriteAcks) error {
			if !req.StreamIncludeUsage() || finalUsage == nil {
				return nil
			}
