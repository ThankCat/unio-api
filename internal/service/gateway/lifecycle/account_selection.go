...
// accountAttemptBudget 限制单个请求在同一个池内最多尝试多少个账号。
...
// 池型候选在这里完成第二阶段选号——按池内顺序逐号 AttemptPermit，账号槽满或账号冷却就换下一个号；
// 账号级拒绝才换号，渠道级拒绝（熔断、渠道冷却、权限暂停、配置不同步）立即返回，
// 因为换哪个号都会得到同一个答案。
//
// attemptedAccounts 是请求级的，跨候选共享：同一账号在单请求内绝不重复尝试（边界 6）。
func (r *AttemptRunner) acquireCandidatePermit(
...
) (breakerstore.AttemptAdmission, *AttemptPermitOwner, accountpool.Member, error) {
	if prepared.AccountPool == nil {
		...
		return admission, owner, accountpool.Member{}, err
	}

	pool := *prepared.AccountPool
	var denials accountDenialSummary
	tried := 0
	for _, accountID := range pool.Order(0, nil) {
		if attemptedAccounts[accountID] {
			continue
		}
		if tried >= accountAttemptBudget {
			break
		}
		...
		if err != nil {
			return admission, owner, member, err
		}
		if admission.Mode == breakerstore.AdmissionPermit {
			return admission, owner, member, nil
		}
		if !accountScopedDenial(admission.Reason) {
			// 渠道级拒绝与账号无关，换号是白跑一趟 Redis。
			return admission, nil, accountpool.Member{}, nil
		}
		denials.record(admission)
	}

	return denials.admission(), nil, accountpool.Member{}, nil
}
...
func (s accountDenialSummary) admission() breakerstore.AttemptAdmission {
	if !s.seen {
		// 池内没有任何号可试（全部在本请求内试过了）：等同于该渠道容量耗尽，
		// 报 concurrency_full 让上层按「换渠道 / 全池短等」处理，而不是伪装成无候选。
