...
func (m *AttemptPermitManager) Acquire(ctx context.Context, params AttemptPermitAcquireParams) (breakerstore.AttemptAdmission, *AttemptPermitOwner, error) {
	...
	in := breakerstore.AcquireAttemptInput{
		...
		OriginRevision:            params.Candidate.OriginRevision,
		ProviderStatusRevision:    params.Candidate.ProviderStatusRevision,
	...
	if result.Mode != breakerstore.AdmissionPermit {
		...
		logging.Debug(m.logger, "admission", "permit", "attempt permit denied", fields...)
	...
	logging.Debug(
		...
		owner.logData(
			zap.Int64("provider_id", params.Candidate.ProviderID),
			zap.Int64("channel_id", params.Candidate.Channel.ID),
			zap.String("endpoint", string(params.UpstreamEndpoint)),
			zap.String("mode", string(params.RequestMode)),
			zap.String("permit_id_hash", permitIDHash(result.Permit.PermitID)),
			zap.Int64("permit_ttl_ms", result.Permit.PermitTTLMs),
			zap.Int64("renew_interval_ms", result.Permit.RenewMs),
		)...,
	)
...
func (m *AttemptPermitManager) recordAcquireRevisionMismatch(reason breakerstore.DeniedReason) {
	...
	case breakerstore.ReasonStaleStatusRevision:
		m.metrics.IncProviderStatusRevisionMismatch("acquire")
	}
}
	...
	o.terminalOnce.Do(func() {
		...
		logging.Debug(
			...
			o.logData(
				...
				zap.String("provider_outcome", string(outcome.ProviderOutcome)),
				zap.String("channel_outcome", string(outcome.ChannelOutcome)),
				zap.String("provider_disposition", string(o.terminalResult.ProviderDisposition)),
				zap.String("channel_disposition", string(o.terminalResult.ChannelDisposition)),
				zap.String("fault_party", breakerFaultParty(outcome)),
			)...,
		)
...
func (o *AttemptPermitOwner) recordRuntimeFeedback(ctx context.Context, upstreamErr error) error {
		...
		if feedbackErr == nil {
			...
			logging.Error(
				...
				o.logData(
					zap.Int64("provider_id", o.permit.ProviderID), zap.Int64("channel_id", o.permit.ChannelID),
					zap.Int64("duration_ms", durationMs), zap.Int64("source_retry_after_ms", sourceRetryAfterMs),
					zap.String("error_message", feedbackErr.Error()),
				)...,
			)
		}
	case category == adapter.UpstreamErrorPermission && metadata.StatusCode == 403:
		...
		feedbackErr = o.runtimeFeedbackStore.PauseChannelModelPermission(
			...
			o.permit.OriginRevision,
			o.permit.ProviderStatusRevision,
		)
		cancel()
		if feedbackErr == nil {
			logging.Warn(
				...
				o.logData(
					...
					zap.Int64("config_revision", o.permit.ChannelConfigRevision),
					zap.Int64("origin_revision", o.permit.OriginRevision),
					zap.Int64("provider_status_revision", o.permit.ProviderStatusRevision),
				)...,
			)
		} else {
			logging.Error(
...
func (o *AttemptPermitOwner) recordFinishResult() {
	...
	result := "mixed"
	if o.terminalResult.ProviderDisposition == o.terminalResult.ChannelDisposition {
		result = string(o.terminalResult.ProviderDisposition)
	} else if o.terminalResult.ProviderDisposition == breakerstore.DispositionApplied ||
		o.terminalResult.ChannelDisposition == breakerstore.DispositionApplied {
		result = "applied"
	}
	o.metrics.IncBreakerPermitOperation("finish", result)
	o.recordFinishDisposition("provider", o.terminalResult.ProviderDisposition)
...
func (o *AttemptPermitOwner) logData(fields ...zap.Field) []zap.Field {
	if o != nil && o.logFields != nil {
		return append(o.logFields.ZapFields(), fields...)
	}
	return fields
}

func permitIDHash(value string) string {
	sum := sha256.Sum256([]byte(value))
	return fmt.Sprintf("%x", sum[:8])
}

func breakerFaultParty(outcome breakerstore.FinishOutcome) string {
	if outcome.ProviderOutcome == breakerstore.OutcomeEligibleFailure {
		return "provider"
	}
	if outcome.ChannelOutcome == breakerstore.OutcomeEligibleFailure {
		return "channel"
	}
	if outcome.ProviderOutcome == breakerstore.OutcomeEligibleSuccess || outcome.ChannelOutcome == breakerstore.OutcomeEligibleSuccess {
		return "none"
	}
...
func nonStreamFinishOutcome(_ AttemptSuccess, timing AttemptTimingFacts, err error) breakerstore.FinishOutcome {
	...
	applyProviderFailureAttribution(&out, timing, false, err)
	return out
}

// applyProviderFailureAttribution 把无需聚合的 Provider 故障直接归因，并把三类歧义故障交给
// BreakerStore.Finish 在 Redis 内按 distinct Channel + model 证据门槛原子判定。
func applyProviderFailureAttribution(
	out *breakerstore.FinishOutcome,
	...
	if code == failure.CodeAdapterReadStreamFailed || code == failure.CodeAdapterStreamIdleTimeout {
		if category == adapter.UpstreamErrorTimeout || (!categoryOK && timeoutError(err)) {
			if stream && timing.FirstTokenMs() == nil {
				out.ProviderEvidence = breakerstore.ProviderEvidenceFirstTokenTimeout
			} else {
				out.ProviderEvidence = breakerstore.ProviderEvidenceBodyReadTimeout
			}
			return
		}
		if category == adapter.UpstreamErrorServer || (!categoryOK && code == failure.CodeAdapterReadStreamFailed) {
			...
			out.ProviderOutcome = breakerstore.OutcomeEligibleFailure
		}
		return
	}
	...
	if category == adapter.UpstreamErrorServer {
		switch statusCode {
		case 500:
			out.ProviderEvidence = breakerstore.ProviderEvidenceHTTP500
		case 502, 503, 504:
			out.ProviderOutcome = breakerstore.OutcomeEligibleFailure
		case 0:
			out.ProviderOutcome = breakerstore.OutcomeEligibleFailure
...
func (r *AttemptRunner) invokeNonStreamAttempt(
	...
	finishOutcome.RequestWriteState = breakerstore.RequestWriteState(facts.RequestWriteState)
	finishOutcome.ResponseHeadersReceived = facts.ResponseHeadersSeen
	finishOutcome.FirstTokenEligible = facts.UpstreamFirstTokenAt != nil
